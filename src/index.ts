...
const PYTH_TOKEN_ADDRESS = new PublicKey(
...
program
  .command("verify")
  .description("Verify a vesting account given its position account address")
  .requiredOption("-o --owner <pubkey>", "Expected owner")
  .requiredOption("-b --balance <number>", "Expected balance")
  .option(
    "-u --url <string>",
    "RPC URL to use",
    "https://mainnet.helius-rpc.com/?api-key=10f312e5-47b7-41b3-8bd1-8aa2f6a6b948",
  )
  .action(async (options: any) => {
    const owner = new PublicKey(options.owner);
    const balance = new BN(removeCommas(options.balance)).mul(
      new BN(10).pow(new BN(6)),
    );

    const provider = new AnchorProvider(
      new Connection(options.url),
      new NodeWallet(new Keypair()),
      AnchorProvider.defaultOptions(),
    );
    const positionAccountAddresses = await getPositionAccounts(
      provider.connection,
      owner,
    );
    ...
    if (positionAccountAddresses.length > 1) {
      console.log(
        `❗Multiple locked accounts found for ${owner.toBase58()}. Please proceed with caution.`,
      );
    }
    ...
    for (let [
    ...
    ] of positionAccountAddresses.entries()) {
      ...
      const positionAccountInfo =
        await stakingProgram.provider.connection.getAccountInfo(
          positionAccountAddress,
        );
      const positionAccountData =
        await stakingProgram.account.positionData.fetch(positionAccountAddress);
      ...
      try {
        ...
        assert(
          ...
          "Position account is not owned by the vesting program",
        );

        assert(positionAccountData, "Position account data not found");
        assert(
          positionAccountData.owner.equals(owner),
          "Position account owner field is not the expected one",
        );

        assert(metadataAccountData, "Metadata account data not found");
        assert(
          metadataAccountData.owner.equals(owner),
          "Metadata account owner field is not the expected one",
        );
        assert(
          metadataAccountData.lock.periodicVestingAfterListing,
          "Metadata account is not periodic vesting after listing",
        );
        assert(
          metadataAccountData.lock.periodicVestingAfterListing.numPeriods.eq(
            new BN(4),
          ),
          "Vesting schedule has the wrong number of periods",
        );
        assert(
          metadataAccountData.lock.periodicVestingAfterListing.periodDuration.eq(
            ONE_YEAR,
          ),
          "Vesting schedule has the wrong period duration",
        );

        assert(custodyAccountData, "Custody account data not found");
        assert(
          custodyAccountData.mint.equals(PYTH_TOKEN_ADDRESS),
          "Custody account mint is not the expected one",
        );

        const targetBalance =
          metadataAccountData.lock.periodicVestingAfterListing.initialBalance;

        if (!targetBalance.eq(balance)) {
          console.log(
            `❌ Specified balance does not match with smart contract balance: contract ${addCommas(
              targetBalance.div(new BN(10).pow(new BN(6))).toString(),
            )} vs specified ${addCommas(options.balance.toString())}`,
          );
        } else {
          console.log(`Succesfully verified vesting account`);
          console.log(
            `for owner ${owner.toBase58()} and balance ${addCommas(
              options.balance,
            )} PYTH`,
          );
          console.log(
            `The custody token account is ${custodyAccountAddress.toBase58()}`,
          );
          if (custodyAccountData.amount.eq(new BN(0))) {
            console.log(
              `✅ Please send ${addCommas(
                options.balance,
              )} PYTH Tokens to \x1b[32m${custodyAccountAddress.toBase58()}`,
            );
            ...
            console.log(
              `❌ This account has received ${custodyAccountData.amount
                .sub(targetBalance)
                .div(new BN(10).pow(new BN(6)))
                .toString()} tokens more than expected`,
            );
          } else if (custodyAccountData.amount.lt(targetBalance)) {
            console.log(
              `✅ This account hasn't received the totality of their tokens. Please send ${addCommas(
                targetBalance
                  .sub(custodyAccountData.amount)
                  .div(new BN(10).pow(new BN(6)))
                  .toString(),
              )} PYTH Tokens to \x1b[32m${custodyAccountAddress.toBase58()}`,
            );
...
async function getPositionAccounts(connection: Connection, owner: PublicKey) {
  const response = await connection.getProgramAccounts(VESTING_PROGRAM_ID, {
    encoding: "base64",
    filters: [
      {
        memcmp: {
